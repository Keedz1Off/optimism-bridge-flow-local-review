...
The first relay must come through the other messenger. If it is not from the other messenger, it must be a replay of a previously failed message.
...
This creates the unique message fingerprint used for replay protection.
...
The message is marked as successful after the target call succeeds.
...
If execution fails, the message is stored in `failedMessages` and can be replayed later.
